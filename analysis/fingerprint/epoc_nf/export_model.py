#!/usr/bin/env python3
"""
export_model.py  (cluster, env eeg_preproc)  —  freeze the EPOC-12 EFP decoder
------------------------------------------------------------------------------
Trains CEN and DMN ridges on ALL DMNELF (clean confound-regressed targets, EPOC-12 electrodes,
[10 band x 11 delay] design, feedback block) and saves a single deployable model:

  epoc_nf/model/efp_epoc_model.npz

Feature/design layout MUST match the online decoder (rt_features.py):
  per channel (EPOC12 order) -> make_delay_design row = [delay0: band0..9, delay1: band0..9, ...],
  channels concatenated channel-major -> 12 * 11 * 10 = 1320 features.
...
Run: python export_model.py --cache <features_cache> --cenmean <cenrel_out> --out <model.npz>
"""
import argparse
...
def subject_designs(runs, ch_names, eidx, n_delays, tv, target):
    """Feedback-masked, per-run z-scored EPOC-12 multivariate design + z-scored clean target."""
    Xs, ys = [], []
    for rd in runs:
        if rd["run"] not in tv:
            continue
        per_ch, off = [], None
        for ci in eidx:                                   # EPOC12 order
...
def main():
    ...
    ap.add_argument("--out", default="/home/cccbauer/efp_epoc_model.npz")
    ...
    for sub in subs:
        ...
        eidx = [ch_names.index(c) for c in EPOC12 if c in ch_names]
        if len(eidx) != len(EPOC12):
            print(f"  {sub}: missing EPOC channels, skip"); continue
    ...
    model = {"channels": np.array(EPOC12), "n_bands": n_bands, "n_delays": n_delays,
             "tr": tr, "sfreq": cfg["data"]["eeg"]["sfreq"], "fmin": fmin, "fmax": fmax,
             "band_edges_hz": band_edges, "layout": "channel-major, delay-major, band-minor"}
    for tgt in ["CEN", "DMN"]:
    ...
    print(f"saved {a.out}  (bands Hz: {band_edges.tolist()})")
