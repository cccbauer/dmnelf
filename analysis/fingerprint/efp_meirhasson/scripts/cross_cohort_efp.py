...
def stack_design(cache, subs, electrode, target, res, n_delays):
    """Concatenate [band x delay] designs at a fixed electrode across subjects."""
    Xs, ys, used = [], [], []
    for sub in subs:
        try:
            ...
            continue
        if electrode not in chs:
            continue
        X, y = assemble(runs, chs.index(electrode), target, res, n_delays)
...
def main():
    ...
    for target in targets:
        ...
        if el is None:
            print(f"{target}: no LOSO electrode, skip"); continue

        Xtr, ytr, tr_used = stack_design(args.dmnelf_cache, dmnelf_subs, el, target, res, n_delays)
        if Xtr is None:
            print(f"{target}: no DMNELF training data at {el}, skip"); continue
        ...
        for sub in rtbpd_subs:
            try:
                runs, chs = load_subject_features(Path(args.rtbpd_cache), sub)
            except Exception:
                continue
            if el not in chs:
                rows.append(dict(cohort="rtBPD", subject=sub, target=target,
                                 electrode=el, n_trs=0, r=np.nan, note="electrode missing"))
                continue
            X, y = assemble(runs, chs.index(el), target, res, n_delays)
            if X is None or len(y) < 10:
                continue
            ...
            subj_rs.append(r)
            rows.append(dict(cohort="rtBPD", subject=sub, target=target,
                             electrode=el, n_trs=len(y), r=r, note=""))
        mean_r = float(np.nanmean(subj_rs)) if subj_rs else np.nan
        p = sign_flip_p(subj_rs)
        summ.append(dict(target=target, resolution=res, electrode=el,
                         n_train=len(tr_used), n_test=len(subj_rs),
                         mean_r=mean_r, sign_flip_p=p))
        print(f"{target:8s} el={el:4s} train={len(tr_used):2d} test={len(subj_rs):2d} "
