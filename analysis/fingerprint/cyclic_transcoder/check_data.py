...
def find_bold(cfg, subject, task):
    ...
    if not func_dir.exists():
        return found
    for bold in sorted(func_dir.glob(
        f"*task-{task}*_space-{space}_res-{res}_desc-preproc_bold.nii.gz"
    )):
        conf = func_dir / bold.name.replace(
...
def find_eeg(cfg, subject, task):
    """Return list of preprocessed 500Hz .fif paths for subject/task."""
    ...
    if not eeg_dir.exists():
        return []
    return sorted(eeg_dir.glob(f"*task-{task}*_desc-{desc}_eeg.fif"))
...
def check_masks(cfg, subject):
    mask_root = cfg["data"]["personal_masks_dir"]
    m_cfg = cfg["data"]["masks"]
    space = m_cfg["space"]
    res   = m_cfg["res"]
    prefix = f"sub-{subject}_space-{space}_res-{res}"
    mask_dir = Path(mask_root) / f"sub-{subject}"

    results = {}
    for label, suffix in [("DMN", m_cfg["dmn_suffix"]), ("CEN", m_cfg["cen_suffix"])]:
        path = mask_dir / f"{prefix}_{suffix}"
        if path.exists():
            try:
                import nibabel as nib
                img = nib.load(str(path))
                n_vox = int((img.get_fdata() > 0).sum())
                results[label] = (True, n_vox, str(path))
            except Exception:
...
def main():
    ...
    for subj in subjects:
        ...
        for task in tasks:
            ...
            # EEG
            eeg_files = find_eeg(cfg, subj, task)
            if eeg_files:
                ...
                issues.append(f"{subj} task={task}: EEG .fif not found")

        # Masks
        masks = check_masks(cfg, subj)
