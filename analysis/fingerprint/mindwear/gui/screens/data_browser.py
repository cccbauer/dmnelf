...
Pick a study; the browser lists every subject with saved data under ``data/<subject>/`` and reports
the two headline R-mbNF metrics: direction-report **accuracy** (the up/down self-test) and PDA
**regulation** (mean feedback-phase z). Parsing lives in :mod:`mindwear.results` (flet-free).
"""
...
from results import SubjectResult, study_results  # noqa: E402
...
class DataBrowser:
    ...
    def _body(self) -> ft.Control:
        ...
        cards = ft.Column([self._subject_card(r) for r in results],
                          spacing=st.GAP, scroll=ft.ScrollMode.AUTO, expand=True)
        return ft.Column([summary, ft.Divider(), cards], spacing=st.GAP, expand=True)
    ...
    def _subject_card(self, r: SubjectResult) -> ft.Control:
        acc = r.accuracy
        chips = ft.Row([
        ...
        detail = ft.ExpansionTile(
            title=ft.Text("Per-run detail", size=st.BODY),
            affinity=ft.TileAffinity.LEADING, controls=[self._run_table(r)])

        return ft.Card(content=ft.Container(
            content=ft.Column([
                ft.Row([ft.Icon(ft.Icons.PERSON, color=st.ACCENT),
