...
ax.set_ylabel("group mean r"); ax.set_title("Within-subject EFP vs baselines (TR resolution, n=19)")
...
for line, sz in [("Replicating Meir-Hasson et al. 2014 on the DMNELF simultaneous EEG–fMRI cohort", 20),
                 ("Single electrode · Stockwell time-frequency · data-driven bands · sliding-delay ridge · n=19", 15)]:
...
bullets(s, [("Replicates the paper's ordering across every target; strongest for CEN, PDA, "
             "GSR_CEN, GSR_PDA (r≈0.14–0.18, TR & 4Hz). GSR_DMN weakest (r≈0.13–0.16), "
             "consistent with our other pipelines.", 0)],
        top=6.4, size=13)
...
bullets(s, [
    ("Within-subject EFP roughly matches or beats PDA decoding (r=0.17 vs DWT 0.11, HMM 0.10) and "
     "GSR_CEN (0.17 vs DWT 0.17 — on par, not an improvement there).", 0, GREEN, True),
    ("Even cross-subject (LOSO) EFP transfers for PDA (r=0.16, p=0.002) — a general fingerprint.", 0),
], top=6.2, size=13)
...
bullets(s, [
    ...
    ("GSR_CEN and GSR_PDA now transfer significantly too (p<.01) with n=19 — only GSR_DMN "
     "remains non-significant.", 1, RED),
...
# 6b — what changed by adding the 2 R128-recovered subjects (n=17 -> n=19)
s = prs.slides.add_slide(BLANK)
title(s, "What changed: recovering 2 more subjects (n=17 → n=19)",
      "sub-dmnelf002/003 had no usable EEG-fMRI alignment until R128 trigger markers were "
      "reconstructed from the gradient artifact — see the DMNELF EEG pipeline notes")
img(s, RES / "fig_n17_vs_n19_loso_tr.png", left=0.5, top=1.35, width=12.3)
bullets(s, [
    ("3 targets went from non-significant/marginal at n=17 to clearly significant at n=19: "
     "PDA (hz4), GSR_CEN (hz4), GSR_PDA (hz4).", 0, GREEN, True),
    ("GSR_CEN and GSR_PDA (tr) both got stronger too (p tightened by 3-4x). Only CEN softened "
     "slightly (still p<.01), with its peak electrode shifting Cz→CP6.", 0),
    ("Net effect of the 2 recovered subjects: real, if modest, improvement — nothing regressed "
     "meaningfully.", 0, NAVY, True),
], top=6.35, size=13)
...
# Positive control — visual cortex (Fig 5 analog): sanity-check that the blind
# EFP pipeline recovers the known occipital-alpha signature.
s = prs.slides.add_slide(BLANK)
title(s, "Positive control — visual cortex (Fig 5 analog)",
      "Blind EFP on an early-visual ROI should recover occipital electrodes + alpha band — validates the method")
img(s, RES / "paper_fig3_composite_VIS_tr.png", left=0.4, top=1.4, width=12.5)

# 9 verdict
s = prs.slides.add_slide(BLANK); title(s, "Verdict")
bullets(s, [
    ("Faithful EFP replication succeeds on DMNELF, now on n=19 (2 subjects recovered via R128 "
     "marker reconstruction).", 0, NAVY, True),
    ("The sliding-delay single-electrode fingerprint is our best DMN/CEN/PDA decoder to date, "
     "within-subject (PDA r≈0.17, CEN r≈0.17).", 1),
    ("A general (LOSO) PDA fingerprint transfers across subjects (r≈0.16, p=0.002) — promising for "
    ...
    ("Adding the 2 recovered subjects only helped: 3 targets went non-significant→significant, "
     "several others tightened; nothing regressed meaningfully.", 1, GREEN),
    ("Caveats", 0, NAVY, True),
