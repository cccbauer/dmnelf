...
def make_loocv_loaders(cfg, left_out_subject, batch_size=8, num_workers=4, seed=None):
    """
    ...
    # Reproducibility: the sampler's shuffling draws from this generator, and each
    # DataLoader worker process forks with its own copy of NumPy's global RNG state
    # (used by the __getitem__ time-shift augmentation) -- worker_init_fn reseeds
    # NumPy/random per worker from torch's per-worker initial_seed so runs with the
    # same --seed are actually reproducible, not just the main process.
    generator = None
    worker_init_fn = None
    if seed is not None:
