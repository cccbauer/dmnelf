# 02_stats_bandpower.py
# Run locally: python 02_stats_bandpower.py
# Deploys cluster script, submits SLURM job, monitors.
#
# Loads band_power.csv (from 01_compute_band_power.py), averages pre
...
# stats_pre_vs_post_bandpower.csv and a bar chart (band_power_prepost.png)
# in the same pre/post bar-chart style as the microstate pipelines.
...
# Fix the Path import placed inline above (keep script self-contained)
lines = ['from pathlib import Path'] + lines

# ── 2. Save cluster script locally ─────────────────────────
script_name = "02_stats_bandpower_cluster.py"
...
# ── 5. Submit SLURM job ────────────────────────────────────
job_name = "rtbpd_stats_bandpower"
sbatch_lines = [
    "#!/bin/bash",
...
sbatch_name = "02_stats_bandpower.sh"
