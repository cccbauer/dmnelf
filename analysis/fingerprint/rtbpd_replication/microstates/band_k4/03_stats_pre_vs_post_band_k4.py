# 03_stats_pre_vs_post_band_k4.py
# Run locally: python 03_stats_pre_vs_post_band_k4.py --band fullspectrum
#              python 03_stats_pre_vs_post_band_k4.py --band theta
# Deploys cluster script, submits SLURM job, monitors.
#
# Adapted from ../03_stats_pre_vs_post_rtbpd.py: paired t-test + paired
# Cohen's d + BH-FDR across 4 microstates x 4 parameters (16 tests) for
# ONE band per invocation -- each band is its own FDR family (16 tests),
# NOT pooled across both bands into 32, per explicit user decision
# (mirrors how the source paper treated bands as separate factors).
# Same confirmatory MixedLM as the k=7 pipeline.
#
# Outputs ({CLUSTER_BASE}/results/):
#   stats_pre_vs_post_{band}_k4.csv
#   templates_topomap_{band}_k4.png   (4-panel, labeled A/B/C/D)
#   pre_vs_post_params_{band}_k4.png
...
from config_band_k4 import (
    ...
    SFREQ_TAG, N_MICROSTATES, BANDS, SCRIPTS_DIR,
)

parser = argparse.ArgumentParser()
parser.add_argument("--band", required=True, choices=BANDS)
parser.add_argument("--time-budget", default="00:45:00",
                    help="SLURM --time (default: 00:45:00)")
...
lines = [
    '#!/usr/bin/env python3',
    '"""',
    '03_stats_pre_vs_post_band_k4_cluster_' + BAND + '.py',
    'Paired pre-vs-post stats (t-test + MixedLM confirmatory) on the',
    'temporal_params_' + TAG + '.csv produced by 02_temporal_params_band_k4.py,',
...
script_name = "03_stats_pre_vs_post_band_k4_" + TAG + "_cluster.py"
...
# ── 5. Submit SLURM job ────────────────────────────────────
job_name = "rtbpd_stats_band_k4_" + BAND
sbatch_lines = [
    "#!/bin/bash",
...
sbatch_name = "03_stats_pre_vs_post_band_k4_" + TAG + ".sh"
