# 02_temporal_params_band_k4.py
# Run locally: python 02_temporal_params_band_k4.py --band fullspectrum
#              python 02_temporal_params_band_k4.py --band theta
# Deploys cluster script, submits SLURM job, monitors.
#
# Adapted from ../02_temporal_params_rtbpd.py: back-fits continuous rest
# EEG (same band filter as step 01 -- theta gets the 4-8 Hz zero-phase FIR
# bandpass BEFORE back-fitting, fullspectrum does not) onto the 4
# templates fit by 01_fit_microstates_band_k4.py for that band, computes
# duration/occurrence/coverage/GEV per (subject, run, microstate).
...
parser.add_argument("--time-budget", default="04:00:00",
                    help="SLURM --time (default: 04:00:00)")
...
lines = [
    '#!/usr/bin/env python3',
    '"""',
    '02_temporal_params_band_k4_cluster_' + BAND + '.py',
    'Back-fit continuous rest EEG (band=' + BAND + ') onto the 4 classic',
...
script_name = "02_temporal_params_band_k4_" + TAG + "_cluster.py"
...
# ── 5. Submit SLURM job ────────────────────────────────────
job_name = "rtbpd_temporal_band_k4_" + BAND
sbatch_lines = [
    "#!/bin/bash",
...
sbatch_name = "02_temporal_params_band_k4_" + TAG + ".sh"
