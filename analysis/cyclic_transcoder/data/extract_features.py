...
def get_bold_path(cfg, subject, task, run=None):
    ...
    # Walk sessions (ses-01, ses-1, etc.) — handle variable naming
    for ses_dir in sorted((fprep / sub).glob("ses-*")):
        func_dir = ses_dir / "func"
        if not func_dir.exists():
            continue
        run_tag = f"_run-{run}" if run is not None else ""
        pattern = (
            f"{sub}_{ses_dir.name}_task-{task}{run_tag}"
        ...
        if bold_path.exists():
            return bold_path, conf_path if conf_path.exists() else None, ses_dir.name
...
def extract_personal_roi_timeseries(bold_img, subject, cfg):
    ...
    Masks must be in MNI152NLin2009cAsym 2mm space.
    """
    mask_dir = Path(cfg["data"]["personal_masks_dir"]) / f"sub-{subject}"
    dmn_path = mask_dir / "DMN.nii.gz"
    cen_path = mask_dir / "CEN.nii.gz"
    ...
    for mask_path, label in [(dmn_path, "DMN"), (cen_path, "CEN")]:
        if not mask_path.exists():
            raise FileNotFoundError(
                ...
                "Make sure masks are in MNI152NLin2009cAsym 2mm space."
            )
...
def find_eeg_path(cfg, subject, task, run=None):
    """Return path to preprocessed MNE .fif file."""
    eeg_root = Path(cfg["data"]["eeg_preproc_dir"])
    sub = f"sub-{subject}"
    run_tag = f"_run-{run}" if run is not None else ""
    # Search sessions
    for ses_dir in sorted((eeg_root / sub).glob("ses-*")):
        eeg_dir = ses_dir / "eeg"
        if not eeg_dir.exists():
            continue
        pattern = f"{sub}_{ses_dir.name}_task-{task}{run_tag}_desc-preproc_eeg.fif"
        p = eeg_dir / pattern
        if p.exists():
            return p, ses_dir.name
    return None, None
