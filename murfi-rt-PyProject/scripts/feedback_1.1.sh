...
clear
    ...
    echo "+ ACCELERATED PROCESSING: Using $(nproc) CPU cores for MELODIC"
    
    # Set FSL parallel processing variables
    export OMP_NUM_THREADS=$(nproc)
    export FSL_CPU_CORES=$(nproc)
    export MELODIC_NPROC=$(nproc)
    
    echo "+ FSL configured for $FSL_CPU_CORES cores"
...
#--add-entry="Second Input Run #"
    ...
    then
        ...
        echo "Using run ${rest_runA_num} and run ${rest_runB_num} with ${FSL_CPU_CORES} cores"
        ...
        echo "+ merging volumes for run A with parallel processing"
        volsA=$(find ${subj_dir_absolute}/img/ -type f \( -iname "img-0000${rest_runA_num}*" ! -iname "*00001.nii" \))
        echo "+ merging volumes for run B with parallel processing"
        ...
        echo "+ computing mean reference volume"
        ...
        echo "+ computing resting state networks with ${FSL_CPU_CORES} CPU cores"
        echo "+ this will take about 5-10 minutes with accelerated processing"
        ...
        # Set FEAT to use multiple cores for MELODIC
        sed -i "s/set fmri(melodic_yn) 1/set fmri(melodic_yn) 1\nset fmri(nproc_melodic) ${FSL_CPU_CORES}/g" $subj_dir_absolute/rest/$subj'_'$ses'_task-rest_'$run'_bold'.fsf
        
        echo "+ launching FEAT with parallel MELODIC processing"
        feat $subj_dir_absolute/rest/$subj'_'$ses'_task-rest_'$run'_bold'.fsf        
    else
        ...
        echo "Using run ${rest_runA_num} for single-run ICA with ${FSL_CPU_CORES} cores"

        # merge individual volumes (except volume 1, see note above!) to make 1 file for each resting state run
        rest_runA_filename=$subj_dir_absolute/rest/$subj'_'$ses'_task-rest_run-01_bold'.nii
        echo "+ merging volumes for single run with parallel processing"
        volsA=$(find ${subj_dir_absolute}/img/ -type f \( -iname "img-0000${rest_runA_num}*" ! -iname "*00001.nii" \))
        fslmerge -tr $rest_runA_filename $volsA 1.2
